                  ...
                  <div class="order-progress">
                    <ul>
                      <li class="first active">
                        ...
                        <div class="info">{{orderinfo.add_time|filterDate('YYYY.MM.DD HH:mm:ss')}}</div>
                      </li>
                      <li :class="orderinfo.status>1?'active':''">
                        <div class="progress">已付款</div>
                        <div v-if="orderinfo.status>1" class="info">{{orderinfo.payment_time|filterDate('YYYY.MM.DD HH:mm:ss')}}</div>
                      </li>
                      <li :class="orderinfo.status>2?'active':''">
                        <div class="progress">已经发货</div>
                        <div v-if="orderinfo.status>2" class="info">{{orderinfo.completeee_time|filterDate('YYYY.MM.DD HH:mm:ss')}}</div>
                      </li>
                      <li class="last" :class="orderinfo.status>3?'active':''">
                        <div class="progress">已完成</div>
                        <div v-if="orderinfo.status>3" class="info">{{orderinfo.complete_time|filterDate('YYYY.MM.DD HH:mm:ss')}}</div>
                      </li>
                    </ul>
                  </div>
                  <div class="form-box accept-box form-box1">
                    <dl class="head form-group">
                      <dd>
                        ...
                        <a v-if="orderinfo.status==1" @click="goPay" class="btn-pay">去付款</a>
                        <a v-if="orderinfo.status>1&&orderinfo.status<4" @click="goReceive" class="btn-pay">签收</a>
                        <!---->
                      </dd>
                    </dl>
                    <dl class="form-group">
...
export default {
  ...
  methods : {
    getOrderDetail() {
      this.$axios.get(`site/validate/order/getorderdetial/${this.$route.params.orderid}`)
      .then(res=>{
    ...
    goPay() {
      this.$router.push(`/payOrder/${this.orderinfo.id}`)
    },
    goReceive() {
      this.$axios.get(`site/validate/order/complate/${this.orderinfo.id}`)
      .then(res=>{
